...
class _StoryScreenState extends ConsumerState<StoryScreen> {
  ...
  Widget _buildSliverAppBar(BuildContext context, bool isScrolled) {
    return SliverAppBar(
      ...
      centerTitle: true,
      leading: IconButton(
        icon: const Icon(Icons.arrow_back_ios_new_rounded, color: Colors.white, size: 20), // 색상 강화
        onPressed: () => Navigator.pop(context),
      ),
      actions: [
        IconButton(
          icon: const Icon(Icons.psychology_rounded, color: Colors.white, size: 22), // 색상 강화
          onPressed: () => _showModelSettings(context),
          tooltip: "AI 엔진 변경",
        ),
        IconButton(
          icon: const Icon(Icons.text_fields_rounded, color: Colors.white, size: 22), // 색상 강화
          onPressed: () => _showFontSettings(context),
          tooltip: "서체 설정",
        ),
        IconButton(
          icon: const Icon(Icons.history_edu_rounded, color: Colors.white, size: 22), // 색상 강화
          onPressed: () {},
        ),
        const SizedBox(width: 8),
      ],

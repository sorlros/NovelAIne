...
void main() {
  group('Frontend Performance & Optimization Tests', () {
    test('Isolate Parsing Performance (Large Dataset)', () async {
      ...
      final List<Map<String, dynamic>> dummyScenes = List.generate(
        50,
        (index) => {
          'id': 'scene_$index',
      ...
      debugPrint('Dataset: 50 scenes, 2000 chars each (total 100,000 chars)');
      debugPrint(
        'Synchronous Parsing (Main Thread): ${stopwatchSync.elapsedMilliseconds} ms',
      );
      debugPrint(
        'Asynchronous Parsing (Isolate): ${stopwatchAsync.elapsedMilliseconds} ms',
      );
      ...
      debugPrint(
        'Parsing logic verified successfully. Isolate successfully offloads work.',
      );
    });
  });
}
